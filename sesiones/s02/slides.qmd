...
<img src="../../imagenes/tidyverse.png" style="width:200px; display:block; margin:0 auto 0.6em;" />
...
## `geom_histogram()`: distribución continua {.smaller}
...
## `geom_bar()`: conteos por categoría {.smaller}
...
#| echo: false
#| fig-height: 2.8
ggplot(pinguinos, aes(x = species)) +
    geom_bar() +
    theme_minimal()
```
...
ggplot(pinguinos, aes(x = species, y = body_mass_g)) +
    geom_boxplot(fill = "lightblue", colour = "steelblue") +
    theme_classic()
```
:::
::::
...
::: nonincremental
- a) `scale_colour_viridis_c()`: `island`  es categórica, pero el mapping es `colour`
- b) `scale_fill_viridis_c()`:  el mapping es `colour`, pero `island` no es continua
- c) `scale_colour_viridis_d()`: el mapping es `colour` e `island` es categórica
