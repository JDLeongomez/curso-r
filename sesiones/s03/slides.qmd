---
title: "Sesión 3"
subtitle: "Manejo y transformación de datos con dplyr y pipes"
...
[⬇ Descargar `encuesta_bruta.xlsx`](https://github.com/JDLeongomez/curso-r/raw/main/sesiones/s03/datos/encuesta_bruta.xlsx)
...
<img src="../../imagenes/dplyr.png" style="position:absolute; bottom:30px; right:30px; width:110px; filter:drop-shadow(0 0 6px rgba(255,255,255,0.4));" />
...
ggplot(encuesta, aes(x = ansiedad_total)) +
    geom_histogram(binwidth = 1, fill = "#225faa", colour = "white") +
    labs(
        title = "Distribución del puntaje GAD-7",
        x     = "Puntaje total (0–21)",
        y     = "Frecuencia"
...
- Guarda el *script* `.R` con tu nombre y [súbelo aquí](https://unbosqueeduco-my.sharepoint.com/:f:/g/personal/jleongomez_unbosque_edu_co/IgCeBz0ZIgE8Q7pb3asBbZiWATaUw6mmJh4-K3UuslEvgiU?e=MozWcw)
...
::: tip-box
[→ Instrucciones completas y criterios de evaluación](../../tareas/tarea1-figura-fea.html)

[→ Otro ejemplo de inspiración (código)](https://gist.github.com/emilyriederer/2bf4f67d7e198f8359b61706c82e42ee)
:::

---

## Inspiración (1 de 2) {.smaller}
...
set.seed(123)
datos_n <- data.frame(
    Genero = rep(c("Hombre", "Mujer"), each = 100),
    Edad   = rnorm(200, 25, 5),
    Estres = rnorm(200, 50, 10),
    Cafe   = sample(1:20, 200, replace = TRUE)
)
ggplot(datos_n, aes(
    x = Edad, y = Estres,
    color = Genero, shape = Genero,
    size = Cafe
)) +
    geom_point(alpha = 1) +
    geom_smooth(
        method = "lm", color = "#00FF00",
        linewidth = 4, se = TRUE
    ) +
    geom_text(aes(label = round(Edad)),
        color = "#FF00FF", angle = 60, size = 3
    ) +
    labs(
        title = "GRAFICA HIPER PROFESIONAL",
        subtitle = "ANALISIS SUPER CIENTIFICO",
        caption = "100% REAL",
        x = "EDaAaAaAD", y = "ESTREeEEeEs"
    ) +
...
ggplot(mtcars, aes(
    ...
    labs(
        title = "GRÁFICA HORRIBLE: Caballos de fuerza vs Consumo",
        x = "Millas por galón (mpg)",
        y = "Caballos de fuerza (hp)",
        color = "Cilindros",
        size = "Peso del vehículo",
        shape = "Marchas"
    ) +
    scale_color_manual(values = c("cyan", "orange", "purple")) +
    scale_size_continuous(range = c(5, 20)) +
    annotate("text",
        x = 15, y = 300,
        label = "MUCHOS DATOS", color = "red",
        size = 10, angle = 45
    ) +
    annotate("text",
        x = 30, y = 50,
        label = "💥 CARROS 💥", color = "darkgreen",
        size = 12, angle = -30
    )
```
:::
