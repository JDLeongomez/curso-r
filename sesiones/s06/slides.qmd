...
2. Guárdalo dentro de tu proyecto `curso_r`, en la carpeta `scripts/`
...
# Título principal {.unnumbered style="font-size:1em; margin:0; border:none; padding:0;"}

## Sección {.unnumbered style="font-size:0.85em; margin:0.2em 0; border:none; padding:0;"}

Texto normal, **negrita** e *cursiva*.
...
Un bloque de código (*chunk*) es una sección del documento donde va código R. Se delimita con tres acentos graves y `{r}`:
...
```markdown
La muestra incluyó `r nrow(gad7_demo)` participantes.
La media del puntaje GAD-7 fue `r round(mean(gad7_demo$gad7), 1)` puntos
(DE = `r round(sd(gad7_demo$gad7), 1)`).
```
...
::: actividad-desc
:::: nonincremental
1. En tu documento Quarto, añade una sección `## Resultados`
2. Inserta un bloque de código con un histograma o gráfica de barras usando los datos de `encuesta_bruta.xlsx` de la Sesión 3 (cárgalos con una ruta relativa desde tu proyecto)
3. Escribe una oración con al menos un valor calculado con código en línea: número de observaciones, una media o una proporción
::::
:::
