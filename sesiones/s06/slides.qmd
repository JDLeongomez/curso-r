...
encuesta <- read_excel("datos/encuesta_bruta.xlsx", skip = 2) |>
  select(-starts_with("...")) |>
  rename_with(~ tolower(gsub(" ", "_", .)), starts_with("GAD")) |>
  mutate(ansiedad_total = gad_1 + gad_2 + gad_3 + gad_4 + gad_5 + gad_6 + gad_7)
```
...
<div style="font-family:monospace; font-size:0.78em; background:#f5f5f3; border-radius:8px; padding:1em 1.2em; line-height:2;">
  ...
  <div style="color:#225faa; padding-left:1em;">mean(ansiedad_total) → <strong>8.3</strong></div>
  ...
  <div style="color:#c0392b; padding-left:1em;">mean(ansiedad_total) → <strong>8.1</strong> ⚠ desactualizado</div>
</div>
```
...
#| echo: false
#| fig-height: 3.2
#| fig-cap: "Figura 1. Distribución del puntaje GAD-7"
encuesta |>
  ggplot(aes(x = ansiedad_total)) +
  geom_histogram(fill = "#225faa", binwidth = 1) +
  labs(x = "Puntaje total GAD-7", y = "Frecuencia") +
  theme_minimal()
```
:::
:::::
...
## El idioma del documento {.smaller}
...
```{=html}
<pre style="background:#f6f8fa; border-radius:4px; padding:0.75em 1em; font-size:0.85em; line-height:1.8; font-family: monospace;"><code>La muestra incluyó &#96;r nrow(encuesta)&#96; participantes.
La media del puntaje GAD-7 fue &#96;r round(mean(encuesta$ansiedad_total), 1)&#96; puntos
(DE = &#96;r round(sd(encuesta$ansiedad_total), 1)&#96;).</code></pre>
```

::: {.fragment}
Al renderizar, eso produce:

> La muestra incluyó `r nrow(encuesta)` participantes. La media del puntaje GAD-7 fue `r round(mean(encuesta$ansiedad_total), 1)` puntos (DE = `r round(sd(encuesta$ansiedad_total), 1)`).
...
![](../../imagenes/latex.svg){width=300 fig-align="center"}
...
::: tip-box
Para consultar la sintaxis en cualquier momento: [→ Guía rápida de Quarto](../../guia-quarto.html)
:::
